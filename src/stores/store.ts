...
type State = {
  ...
  // Coins (now represents game balance from blockchain)
  coins: number;
  updateCoins: (amount: number) => void;
  
  // Blockchain integration (always enabled)
  ...
  subscribeWithSelector((set) => ({
    ...
    updateCoins: (amount: number) => {
      set((state) => {
        ...
        return {
          coins: newCoins,
          bet: state.bet > newCoins ? Math.min(newCoins, 1) : state.bet,
        };
      });
    },
    ...
    syncWithBlockchain: async () => {
      ...
      try {
        ...
        if (address) {
          const gameBalance = await blockchainService.getGameBalance(address);
          const balanceInCoins = Math.floor(parseFloat(gameBalance) * 100); // Convert IRYS to coins (1 IRYS = 100 coins)
          
          const state = useGame.getState();
          set(() => ({
            coins: balanceInCoins,
